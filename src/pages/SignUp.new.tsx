...
export default function SignUp() {
  ...
  useEffect(() => {
    ...
    if (token && step !== 'password' && !isEmailVerified) {
      setVerificationToken(token);
      // Also set email if passed (optional enhancement)
      (async () => {
        try {
          setIsLoading(true);
          const resp = await fetch('http://localhost:5000/auth/verify-email', {
            method: 'POST',
  ...
  }, [searchParams, step, isEmailVerified]);
        ...
        >
          ...
          {step === 'verify' && (
            <div className="text-center py-4">
              ...
              {/* For development: Show verify button */}
              {verificationToken && (
                <div className="mb-3 p-2 rounded-xl bg-yellow-500/10 border border-yellow-500/30">
                  <p className="text-xs text-yellow-300 mb-2">Development Mode: Click to verify</p>
                  <button
                    onClick={handleVerifyEmail}
                    disabled={isLoading}
                    className="w-full py-1.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold transition-all disabled:opacity-50"
                  >
